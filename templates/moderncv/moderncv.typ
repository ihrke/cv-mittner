...
#let __coverletter_footer(author, language, date, lang_data) = {
  let lang_dict = lang_data.lang.at(language, default: lang_data.lang.en)
  set text(
    fill: gray,
    size: 8pt,
  )
  __justify_align_3[
    ...
    #smallcaps[
      ...
      #sym.dot.c
      #lang_dict.at("cover-letter", default: "Cover Letter")
    ]
  ][
    #context {
...
#let __resume_footer(author, language, lang_data, date) = {
  ...
  __justify_align_3[
    ...
    #smallcaps[
      ...
      #sym.dot.c
      #lang_dict.at("resume", default: "Résumé")
    ]
  ][
    #context {
      counter(page).display()
...
#let resume(
  ...
  show: body => context {
    set document(
      ...
      title: lang_dict.at("resume", default: "Résumé"),
    )
    body
  }
...
//    font: font,
  ...
  set page(
    paper: "a4",
    margin: (left: 15mm, right: 15mm, top: 10mm, bottom: 15mm),
...
/// Publication item with hanging indentation for numbered publications
/// - body (content): The publication entry content including number and text
#let pub-item(body) = {
  set text(
    size: 10pt,
    style: "normal",
  ...
  show link: set text(font: "Fira Code", size: 8pt, fill: rgb("#3734df"))
  par( hanging-indent: 2em, first-line-indent: 0em, justify: true, body)
}

/// Presentation item for formatting conference presentations
/// - conference (string): The conference name
/// - location (string): The location (city, country)
/// - date (string): The date of the presentation
/// - title (string or none): The presentation title (optional)
#let presentation-item(
  ...
  block(above: 1em, below: 0.65em)[
    #pad[
      #__justify_align(conference, location)
      #if conference != "" or date != "" [
        #secondary-justified-header(title, date)
      ]
    ]
  ]
...
/// Condensed student entry for supervision section
/// - name (content): Student name(s) - can include line breaks for multiple names
/// - year (string): Year or year range
/// - title (string): Thesis/project title
/// - role (string): main or co-supervisor (optional)
/// - link (string): URL to thesis/publication (optional)
#let student-entry(
  name: none,
  year: "",
  title: none,
  ...
  block(above: 0.6em, below: 0.4em)[
    #grid(
      columns: (4.5cm, 1fr, auto),
      column-gutter: 8pt,
      [
        #text(size: 10pt, weight: "medium", fill: color-darkgray)[#name]
      ],
      [
      ...
      [
        #align(right)[
          #text(size: 10pt, weight: "regular", fill: color-gray)[#year]
        ]
      ]
    )
  ]
...
/// Generic dated entry with title, optional amount, and description
/// Useful for grants, activities, responsibilities, etc.
/// - title (string): The main text/description
/// - date (string): The date or year
/// - amount (string): Optional additional info (e.g., funding amount)
/// - description (content): Optional bullet points or details
...
#let dated-entry(
  ...
  block(above: 1em, below: 0.65em)[
    #grid(
      ...
      gutter: 0pt,
      [
        #text(size: 10pt, weight: "regular", fill: color-darkgray)[#title]
        #if amount != "" [
          #text(size: 10pt, weight: "regular", fill: color-gray)[ (#amount)]
        ]
        #if description != none [
          #v(0em)
          #text(size: 10pt, weight: "regular", fill: color-darkgray)[#description]
        ]
        #if url != none [
          \ #text(font: "Fira Code", size: 8pt, fill: rgb("#3734df"))[#link(url)[#url]]
        ]
      ...
      [
        #align(right)[
          #text(size: 10pt, weight: "regular", fill: color-gray)[#date]
        ]
      ]
    )
  ]
